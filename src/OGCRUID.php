...
use OG\OGCRUD\Actions\DeleteAction;
use OG\OGCRUD\Actions\EditAction;
use OG\OGCRUD\Actions\RestoreAction;
use OG\OGCRUD\Actions\ViewAction;
use OG\OGCRUD\Events\AlertsCollection;
use OG\OGCRUD\FormFields\After\HandlerInterface as AfterHandlerInterface;
use OG\OGCRUD\FormFields\HandlerInterface;
use OG\OGCRUD\Models\Category;
use OG\OGCRUD\Models\DataRow;
use OG\OGCRUD\Models\DataType;
use OG\OGCRUD\Models\Menu;
use OG\OGCRUD\Models\MenuItem;
use OG\OGCRUD\Models\Page;
use OG\OGCRUD\Models\Permission;
use OG\OGCRUD\Models\Post;
use OG\OGCRUD\Models\Role;
use OG\OGCRUD\Models\Setting;
use OG\OGCRUD\Models\Translation;
use OG\OGCRUD\Models\User;
use OG\OGCRUD\Traits\Translatable;
...
class OGCRUD
{
    ...
    {
        $widgetClasses = config('ogcrud.dashboard.widgets');
        $dimmerGroups = [];
        $dimmerCount = 0;
        $dimmers = Widget::group("ogcrud::dimmers-{$dimmerCount}");
        ...
        foreach ($widgetClasses as $widgetClass) {
            ...
            if ($widget->shouldBeDisplayed()) {
                ...
                if ($dimmerCount % 3 === 0 && $dimmerCount !== 0) {
                    ...
                    $dimmers = Widget::group("ogcrud::dimmers-{$dimmerGroupTag}");
                }
    ...
    {
        $globalCache = config('ogcrud.settings.cache', false);
    ...
    {
        if (!empty($file)) {
            return str_replace('\\', '/', Storage::disk(config('ogcrud.storage.disk'))->url($file));
        }
    ...
    {
        require __DIR__.'/../routes/ogcrud.php';
    }
    ...
    protected function findVersion()
    {
        ...
        if ($this->filesystem->exists(base_path('composer.lock'))) {
            ...
            // Loop through all the packages and get the version of ogcrud
            foreach ($file->packages as $package) {
                if ($package->name == 'og/ogcrud') {
    ...
    {
        if (!config('ogcrud.multilingual.enabled')) {
            return false;
        }
    ...
    {
        $appLocales = [];
        if ($this->filesystem->exists(resource_path('lang/vendor/ogcrud'))) {
            $appLocales = array_diff(scandir(resource_path('lang/vendor/ogcrud')), ['..', '.']);
        }
