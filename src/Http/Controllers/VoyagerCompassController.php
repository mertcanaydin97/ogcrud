...
class VoyagerCompassController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!\App::environment('local') && !config('ogcrud.compass_in_production', false)) {
        ...
        if ($this->request->input('download')) {
            ...
            return redirect($this->request->url().'?logs=true')->with([
                'message'    => __('ogcrud::compass.logs.delete_success').' '.base64_decode($this->request->input('del')),
                'alert-type' => 'success',
            ]);
            ...
            return redirect($this->request->url().'?logs=true')->with([
                'message'    => __('ogcrud::compass.logs.delete_all_success'),
                'alert-type' => 'success',
            ]);
        }
        ...
        return view('ogcrud::compass.index', compact('logs', 'files', 'current_file', 'active_tab', 'commands', 'artisan_output'))->with($message);
    }
